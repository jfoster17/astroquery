...
from . import VISTA_SERVER, VISTA_TIMEOUT
...
def validate_frame(func):
    def wrapper(*args, **kwargs):
        ...
        if frame_type not in VistaClass.frame_types:
            raise ValueError("Invalid frame type. Valid frame types are: {!s}"
                             .format(VistaClass.frame_types))
...
def validate_filter(func):
    def wrapper(*args, **kwargs):
        ...
        if waveband not in VistaClass.filters:
            raise ValueError("Invalid waveband. Valid wavebands are: {!s}"
                             .format(VistaClass.filters.keys()))
        return func(*args, **kwargs)
    return wrapper


class VistaClass(QueryWithLogin):

    """
    The VISTAQuery class.  Must instantiate this class in order to make any
    queries.  Allows registered users to login, but defaults to using the
    public VISTA data sets.
    """
    BASE_URL = VISTA_SERVER()
    ...
    databases = ("VHSDR2", "VHSDR1", "VVVDR1",
                 "VMCDR1", "VMCDR2", "VIKINGDR2",
                 "VIKINGDR3", "VIDEODR2", "VIDEODR3")

    def __init__(self, username=None, password=None, community=None,
                 database='VVVDR1', programme_id='all'):
        self.database = database
        self.programme_id = programme_id  # 120 = VVV
        self.session = None
    ...
    def get_images(self, coordinates, waveband='all', frame_type='stack',
        ...
        """
        Get an image around a target/ coordinates from VISTA catalog.
    ...
    def query_region(self, coordinates, radius=1 * u.arcmin,
                     programme_id='VVV', database='VVVDR1',
    ...
    def query_region_async(self, coordinates, radius=1 * u.arcmin,
                           programme_id='VVV',
                           database='VVVDR1', get_query_payload=False,
                           system='J2000'):
        """
    ...
    def _parse_result(self, response, verbose=False):
        ...
        try:
            ...
            raise
            raise TableParseError("Failed to parse VISTA votable! The raw response can be found "
    ...
    def list_catalogs(self, style='short'):
        """
        Returns a lsit of available catalogs in VISTA.
    ...
    def list_databases(self):
        """
        List the databases available from the VISTA WFCAM archive
        """
...
def _parse_dimension(dim):
    """
    Parses the radius and returns it in the format expected by VISTA.
